...
async function handleApiRequest(req, res, requestUrl) {
  ...
  if (req.method === 'GET' && requestUrl.pathname === '/api/bootstrap') {
    ...
    const [account, chargeStatus, priceInfo, records, stations] = await Promise.all([
      getAccountInfo(auth.session.userid),
      getChargeStatus(auth.session.userid).catch(() => null),
      getPriceInfo(auth.session.userid).catch(() => null),
  ...
  if (req.method === 'GET' && requestUrl.pathname === '/api/jacount') {
    const auth = requireSession(req, res);
    if (!auth) {
      return;
    }
    return sendJson(res, 200, await getJacount(auth.session.userid));
  }
  ...
  if (req.method === 'POST' && requestUrl.pathname === '/api/message') {
    ...
    return sendJson(
      res,
      200,
      await getMessage(auth.session.userid, auth.session.profile),
    );
  }
  ...
  if (req.method === 'GET' && requestUrl.pathname === '/api/charge-list') {
    return sendJson(res, 200, await getChargeList());
  }
  ...
  if (req.method === 'GET' && requestUrl.pathname === '/api/price-info') {
    const auth = requireSession(req, res);
    if (!auth) {
      return;
    }
    return sendJson(res, 200, await getPriceInfo(auth.session.userid));
  }
  ...
  if (req.method === 'POST' && requestUrl.pathname === '/api/pay/apply') {
    const auth = requireSession(req, res);
    if (!auth) {
      return;
    }
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      return sendError(res, 400, '充值金额不合法');
    }

    return sendJson(res, 200, await payApply(auth.session.userid, amount));
  }

  if (req.method === 'POST' && requestUrl.pathname === '/api/pay/return') {
    const auth = requireSession(req, res);
    if (!auth) {
      return;
    }
    ...
    const body = await readJsonBody(req);
    let returnContent = body.returnContent;

    if (typeof returnContent !== 'string') {
      returnContent = JSON.stringify(returnContent || {});
    }

    return sendJson(
      res,
      200,
      await payApplyReturn(auth.session.userid, returnContent),
    );
  }
