...
> Web 优先的校园电动车充电工具，兼容东晟校园生活接口

## 📝项目定位
CampusLifeForCharging 现在以手机 Web 为主线，核心目标是把日常最常用的动作放进浏览器里：

- 手机浏览器直接登录校园账户
- 查看余额、当前充电状态和月度记录
- 查看所有地点与每一根充电桩的实时状态
- 浏览器扫码或手动输入编号发起充电
- 将 Web 服务部署到自己的服务器上独立运行

充值能力目前仍然依赖原生 App。仓库里保留 Android / iOS 工程，主要用于兼容原有客户端能力、补足平台特性和后续壳层探索。

## 当前推荐使用方式

1. 把 `web/` 部署到自己的服务器，作为日常使用入口。
2. 在线下场景里，用手机浏览器打开 Web 完成登录、查状态和充电。
3. 需要充值时，回到原生 App 或模拟器中的原生 App 完成付款，再回 Web 刷新余额。

## 来源说明
本项目由 AltCampusLife 的代码和逆向结果继续演化而来，仍与东晟校园生活协议兼容。

![ic_launcher](https://github.com/user-attachments/assets/b1cfe3a4-8e45-4ceb-a8eb-dfe4af9f2afc)

- 🌐 Web 端已经可以独立部署和独立运维
- 📱 手机端日常可直接用浏览器完成主要流程
- 🔌 支持地点总览、逐桩状态、扫码充电和手动充电
- 🧰 Android / iOS 工程仍保留在仓库中，便于继续验证和扩展
