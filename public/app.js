...
function renderStations() {
  ...
  if (!overview) {
    ...
    elements.stationResultsMeta.textContent = '还没有地点数据，稍后可点“刷新列表”再试。';
    return;
  }

  elements.stationNote.textContent =
    overview.note || '这里会展示各地点和充电桩的当前状态，逐桩列表默认折叠，方便手机上快速浏览。';
...
function buildStationResultsMeta(filtered, totalCount, availableCount) {
  ...
  return `${prefix}当前显示 ${visibleCount}/${totalCount || 0} 个地点，其中有空位的地点共 ${availableCount} 个。`;
}
